...
$(function () {
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.code == 200) {
                $("#product_category").append("<option value=''>请选择</option>");
                $.each(data.data, function (idx, obj) {
                    $("#product_category").append("<option value='"+obj.id+"'>"+obj.name+"</option>");
                })
            }
        }
    });
    ...
    $("#product_category").change(function () {
        var text = $("#product_category").find("option:selected").text();
